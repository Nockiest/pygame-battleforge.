...
class ButtonBar:
    def __init__(self, callback):
        ...
        self.button_spacing = remaining_space // (len( self.button_instances) + 1)
    ...
    def update_button_positions(self):
        total_button_width = sum(self.button_width for button in self.button_instances) + (len(self.button_instances) - 1) * self.button_spacing
    ...
    def draw(self, screen, y, current_player_color):
        ...
        for button in self.button_instances:
            button.x = x
            button.y = y
            button.rect = pygame.Rect(button.x, button.y, button.width, button.height)  # Corrected the order of width and height
