...
class Observer(Support):
    ...
    def __init__(self, x, y,  color):
        ...
        super().__init__(hp=1, attack_range=100,attack_resistance=0.05, base_actions=1,  base_movement=50,
                         icon="observer.png", size=self.size, x=x, y=y,  color=color, cost=self.cost)
    ...
    def provide_attack_range(self, selected_unit):
        ...
        if issubclass(selected_unit.__class__, Ranged)   and self.color == selected_unit.color:
            ...
            if distance <= self.attack_range:
                selected_unit.attack_range_modifiers["in_observer_range"] = 1
                return True
            else:
                return False
